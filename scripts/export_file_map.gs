function exportFileMap() {
  var folderId = "1VuY6jM-a8UJi4hHs1sJ6z30O2dYjksl1"; // Replace with your folder ID
  ...
  while (files.hasNext()) {
    var file = files.next();
    var name = file.getName(); // e.g., "arunsabm.pdf"
    var userid = name.replace(/\.[^/.]+$/, ""); // remove file extension
    var fileId = file.getId(); // Correct File ID
    csvLines.push([userid, fileId]);
  }
